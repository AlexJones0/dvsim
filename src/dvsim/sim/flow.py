...
class SimCfg(FlowCfg):
    ...
    def _fake_policy(self, _all_jobs: list[JobSpec], job: JobSpec) -> JobStatus:
        """Tell the fake backend how to fake jobs for this flow.

        Currently randomly returns 50% pass / 50% fail for RunTest jobs only.
        """
        # TODO: faking coverage report results requires writing a file that can be parsed
        # as if it's generated by the tool.
