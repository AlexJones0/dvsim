...
class RuntimeBackend(ABC):
    ...
    async def _emit_completion(self, events: Sequence[JobCompletionEvent]) -> None:
        """Mark a job as in some completed/terminal state, notifying the scheduler."""
    ...
    @abstractmethod
    async def submit_many(self, jobs: Sequence[JobSpec]) -> dict[Hashable, JobHandle]:
        """Submit & launch multiple jobs.

        Returns:
            mapping from job.id -> JobHandle.
    ...
    @abstractmethod
    async def kill_many(self, handles: Sequence[JobHandle]) -> None:
    ...
    def _build_job_env(
        ...
        Arguments:
            ...
            remove: A sequence of variables to remove from the final environment variable list.
              Defaults to None.
        ...
        if job.odir and job.odir.exists():
            err_type = "surrogateescape"
            with (job.odir / ENV_DUMP_PATH).open("w", encoding="utf-8", errors=err_type) as f:
...
class LogResults:
    ...
    def get_status_from_logs(self) -> tuple[JobStatus | None, JobStatusInfo | None]:
        ...
        # TODO: should this be restricted to per-line patterns? This is probably for the
        # line number parsing in the failures, but maybe we can do this in a more sensible way?
