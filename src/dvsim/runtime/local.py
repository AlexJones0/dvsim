...
class LocalRuntimeBackend(RuntimeBackend):
    ...
    async def _log_from_pipe(
        ...
        """Write piped asyncio subprocess stream contents to the job's log file."""
    ...
    def _send_kill_signal(self, proc: asyncio.subprocess.Process, signal: int) -> None:
        """Send a (kill) signal to a process and all it's descendent processes."""
    ...
    async def kill_many(self, handles: Sequence[JobHandle]) -> None:
        ...
        for handle in handles:
            if not isinstance(handle, LocalJobHandle):
                type_ = "LocalJobHandle"
                msg = f"Local backend expected handle of type `{type_}`, not `{type(handle)}`."
                raise TypeError(msg)
