...
class StatusPrinter(ABC):
    ...
    def __init__(self, jobs: Sequence[JobSpec], print_interval: int | None = None) -> None:
        ...
        self._update_cause: str | None = None
    ...
    async def _run(self) -> None:
        ...
        self.update_all_targets(force_all=True)

        while True:
            now = time.monotonic()
            sleep_time = max(0, next_tick - now)
            try:
                await asyncio.wait_for(self._update_event.wait(), timeout=sleep_time)
                self.update_all_targets(cause=self._update_cause)
                self._update_event.clear()
                self._update_cause = None
            except asyncio.TimeoutError:
                self.update_all_targets()
                next_tick += self._interval

    def update_all_targets(self, cause: str | None = None, *, force_all: bool = False) -> None:
        ...
        for target in self._target_counts:
            if (
                force_all
                or target == cause
                or (not self.target_is_done(target) and self.target_is_started(target))
            ):
                self._update_target(update_time, target)
    ...
    def update_status(self, job: JobSpec, old_status: JobStatus, new_status: JobStatus) -> None:
        ...
        if self.target_is_done(job.target) and not self.updates_every_event:
            # Even if we have a configured print interval, we should update if the
            # target is done so we can capture the accurate end timing.
            self._update_cause = job.target
            self._update_event.set()
        elif self.updates_every_event:
            self.update_all_targets(job.target)
...
class EnlightenStatusPrinter(TtyStatusPrinter):
    ...
    def __init__(self, jobs: Sequence[JobSpec]) -> None:
        ...
        if self._interval < self.DEFAULT_MIN_DELTA:
            # TODO: maybe "debounce" the updates with an delayed async refresh task?
            log.warning(
