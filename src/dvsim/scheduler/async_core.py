...
ALL_FAILED_DEP = JobStatusInfo(
    message="Job killed because all of its dependencies failed or were killed."
)
...
class Scheduler:
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """Construct a new scheduler to run a DAG of given jobs.

        Args:
            jobs: The DAG of jobs to run. A sequence of job specifications, where the DAG is
              defined by the job IDs and dependency lists.
            ...
            max_parallelism: The maximum number of jobs that the scheduler is allowed to dispatch
              at once, across all backends. A value of `0` indicates no upper limit.
            priority_fn: A function to calculate the priority of a given job. If no function is
              given, this defaults to using the job's defined weight.
            coalesce_window: Time in seconds to wait on receiving a job completion event, if
              specified, to give a short amount of time to allow other batched completion events
              to arrive in the queue. This lets us batch scheduling more frequently for a little
              extra cost. Defaults to 1 millisecond, can be disabled by giving `None`.
        ...
        self._backend_running: dict[str, int] = dict.fromkeys(backends, 0)
        self._event_queue: asyncio.Queue[Sequence[JobCompletionEvent]] = asyncio.Queue()
    ...
    def _default_priority(self, job: JobRecord) -> float | int:
    ...
    def _mark_job_ready(self, job: JobRecord) -> None:
        ...
        if job.status != JobStatus.SCHEDULED:
            msg = f"_mark_job_ready only works for 'SCHEDULED' jobs (not '{job.status.name}')."
    ...
    def _mark_job_running(self, job: JobRecord) -> None:
        ...
        if job.handle is None:
            raise RuntimeError("_mark_job_running called on a job without an associated handle.")

        self._change_job_status(job, JobStatus.RUNNING)
        self._running.add(job.spec.id)
        self._backend_running[job.backend_key] += 1
    ...
    def _mark_job_completed(
        ...
        if job.kill_requested:
            if self._shutdown_signal == SIGTERM:
                reason = KILLED_RUNNING_SIGTERM
            else:
                reason = KILLED_RUNNING_SIGINT
        self._change_job_status(job, status, reason)

        # If the job was running, mark it as no longer running.
        if job.spec.id in self._running:
            if job.handle is None:
                raise RuntimeError("Running job has no associated handle?")
            self._running.remove(job.spec.id)
            self._backend_running[job.backend_key] -= 1

        # Update dependents (job that depend on this job), propagating failures if needed.
    ...
    async def _submit_job_completion(self, events: Sequence[JobCompletionEvent]) -> None:
            ...
            while True:
                ...
                if not self._running:
                    if not self._ready_heap:
                        break
                    # This case (none running, but jobs still pending in the queue) can happen if
                    # backends fail to schedule any jobs (e.g. the backend is temporarily busy).
                    continue
                ...
                events = await self._drain_completions(job_completion_task)
                job_completion_task = asyncio.create_task(self._event_queue.get())
    ...
    async def _handle_exit_signal(self) -> None:
        ...
        for job_id in self._running:
            ...
            if job.handle is None:
                raise RuntimeError("Running job unexpectedly does not have an associated handle.")
            job.kill_requested = True
            to_kill[job.backend_key].append(job.handle)

        # Asynchronously dispatch batched backend kill tasks whilst we update scheduler internals.
        # Jobs that depend on these jobs will then be transitively killed before starting.
    ...
    async def _schedule_ready_jobs(self) -> None:
        ...
        # Temporarily collect jobs to launch in a dict, grouped per backend, for batched launching.
        ...
        while self._ready_heap and slots_used < available_slots:
            ...
            backend_run = self._backend_running[job.backend_key] + len(to_launch[job.backend_key])

            # Check that we can launch the job whilst respecting backend parallelism limits
            if backend.max_parallelism and backend_run >= backend.max_parallelism:
        ...
        for jobs, handles in zip(to_launch.values(), results, strict=True):
            for neg_priority, job in jobs:
                handle = handles.get(job.spec.id, None)
                if handle is None:
